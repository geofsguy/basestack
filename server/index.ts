...
async function generateWebsiteContent(userData: UserData, profileTree?: ProfileTree | null) {
  ...
  const siteContent = JSON.parse(text) as { html: string };
  if (userData.photos && userData.photos.length > 0) {
    let html = siteContent.html;
    userData.photos.forEach((dataUrl, index) => {
      html = html.split(`__PHOTO_${index + 1}__`).join(dataUrl);
    });
...
async function refineWebsiteContent(currentHtml: string, refinementPrompt: string) {
  const prompt = `
...
app.post('/api/ai/generate', async (req, res) => {
  try {
    ...
    const content = await generateWebsiteContent(userData, profileTree);
    res.json(content);
  } catch (error) {
    sendError(res, error);
  }
...
app.post('/api/ai/refine', async (req, res) => {
  try {
    ...
    const { currentHtml, refinementPrompt } = req.body as { currentHtml?: string; refinementPrompt?: string };
    if (!currentHtml || !refinementPrompt) {
      res.status(400).json({ error: 'Missing refinement payload.' });
      return;
    }

    const content = await refineWebsiteContent(currentHtml, refinementPrompt);
    res.json(content);
  } catch (error) {
    sendError(res, error);
  }
